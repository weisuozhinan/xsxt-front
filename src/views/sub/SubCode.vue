  ...
  <div>
      ...
      <div style="display: flex;justify-content: space-between">
        <div>
          <el-input style="width: 300px;margin-right: 10px" prefix-icon="el-icon-search"
                    v-model="subName" @keydown.enter.native="initCode" clearable @clear="initCode"
                    placeholder="请输入课题名称进行搜索..."></el-input>
          <el-button type="primary" icon="el-icon-search" @click="initCode">搜索
          </el-button>
        </div>
      </div>
    </div>
    <div style="margin-top: 10px">
      <el-table :data="subs"
        ...
        <el-table-column prop="file.codeTitle" label="状态" align="left">
          <template slot-scope="scope">
            <el-tag type="danger" v-if="scope.row.file.codeTitle==null||scope.row.file.codeTitle==''">待上传
            </el-tag>
            <el-tag type="success" v-else>已上传</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="file.codeTitle" label="文件"></el-table-column>
        <el-table-column prop="file.codeCreateTime" label="创建时间" align="left">
          <template slot-scope="scope">
            <p v-if="scope.row.file.codeTitle!=null&&scope.row.file.codeTitle!=''">
              {{ scope.row.file.codeCreateTime }}</p>
          </template>
        </el-table-column>
        <el-table-column prop="file.codeScore" label="成绩" align="left"></el-table-column>
        <el-table-column label="操作" fixed="right" align="left" width="200px">
          <template slot-scope="scope">
            ...
            <el-button style="padding:3px" size="mini" type="primary" @click="exportData(scope.row)"
                       v-if="scope.row.file.codeTitle!=0&&scope.row.file.codeTitle!=null">下载
            </el-button>
            <el-button style="padding:3px" size="mini" type="danger"
                       @click="deleteCodeFile(scope.row)"
                       v-if="scope.row.file.codeTitle!=0&&scope.row.file.codeTitle!=null">删除
            </el-button>
            <el-button style="padding:3px" size="mini" type="danger"
    ...
    <el-dialog
      ...
      <div>
        <el-form ref="codeForm"
          ...
          <el-row>
            <el-col :span="9">
              <el-form-item label="代码检查成绩：" prop="codeScore">
                <el-input size="small" style="width: 150px" v-model="updateFile.codeScore"
...
export default {
  ...
  methods: {
    exportData(data){
      this.downloadRequest('/file/export/code/?fileId='+data.id);
    },
    doUpdateScore() {
      //编辑代码检查成绩信息
    ...
    deleteCodeFile(data) {
      this.$confirm('此操作将永久删除代码检查文件，是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.deleteRequest('/file/delete/codeFile/' + data.id).then(resp => {
          if (resp) {
            this.initCode();
          }
        })
    ...
    updateUrl(data) {
      this.sub = data;
      this.antionUrl = '/file/upload/codeFile?subjectId=' + data.id + '&';
    },
