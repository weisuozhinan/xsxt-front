  ...
  <div>
      ...
      <div style="display: flex;justify-content: space-between">
        <div>
          <el-input style="width: 300px;margin-right: 10px" prefix-icon="el-icon-search"
                    v-model="subName" @keydown.enter.native="initDema" clearable @clear="initDema"
                    placeholder="请输入课题名称进行搜索..."></el-input>
          <el-button type="primary" icon="el-icon-search" @click="initDema">搜索
          </el-button>
        </div>
      </div>
    </div>
    <div style="margin-top: 10px">
      <el-table :data="subs"
        ...
        <el-table-column prop="file.demandTitle" label="状态" align="left">
          <template slot-scope="scope">
            <el-tag type="danger" v-if="scope.row.file.demandTitle==null||scope.row.file.demandTitle==''">待上传
            </el-tag>
            <el-tag type="success" v-else>已上传</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="file.demandTitle" label="文件"></el-table-column>
        <el-table-column prop="file.demandCreateTime" label="创建时间" align="left">
          <template slot-scope="scope">
            <p v-if="scope.row.file.demandTitle!=null&&scope.row.file.demandTitle!=''">
              {{ scope.row.file.demandCreateTime }}</p>
          </template>
        </el-table-column>
        <el-table-column prop="file.demandScore" label="成绩" align="left"></el-table-column>
        <el-table-column label="操作" fixed="right" align="left" width="200px">
          <template slot-scope="scope">
            ...
            <el-button style="padding:3px" size="mini" type="primary" @click="exportData(scope.row)"
                       v-if="scope.row.file.demandTitle!=0&&scope.row.file.demandTitle!=null">下载
            </el-button>
            <el-button style="padding:3px" size="mini" type="danger"
                       @click="deleteDemandFile(scope.row)"
                       v-if="scope.row.file.demandTitle!=0&&scope.row.file.demandTitle!=null">删除
            </el-button>
            <el-button style="padding:3px" size="mini" type="danger"
    ...
    <el-dialog
        ...
        width="30%">
      <div>
        <el-form ref="demaForm"
                 :model="updateFile"
                 :rules="rules">
          <el-row>
            <el-col :span="9">
              <el-form-item label="需求文档成绩：" prop="demandScore">
                <el-input size="small" style="width: 150px" v-model="updateFile.demandScore"
...
export default {
  ...
  methods: {
    exportData(data){
      this.downloadRequest('/file/export/demand/?fileId='+data.id);
    },
    doUpdateScore() {
      //编辑需求文档成绩信息
      this.$refs.demaForm.validate((valid) => {
        if (valid) {
    ...
    deleteDemandFile(data) {
      this.$confirm('此操作将永久删除需求文档文件，是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.deleteRequest('/file/delete/demandFile/' + data.id).then(resp => {
          if (resp) {
            this.initDema();
          }
        })
    ...
    updateUrl(data) {
      this.sub = data;
      this.antionUrl = '/file/upload/demandFile?subjectId=' + data.id + '&';
    },
