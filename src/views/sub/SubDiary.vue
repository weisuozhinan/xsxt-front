  ...
  <div>
      ...
      <div style="display: flex;justify-content: space-between">
        <div>
          <el-input style="width: 300px;margin-right: 10px" prefix-icon="el-icon-search"
                    v-model="subName" @keydown.enter.native="initDiary" clearable @clear="initDiary"
                    placeholder="请输入课题名称进行搜索..."></el-input>
          <el-button type="primary" icon="el-icon-search" @click="initDiary">搜索
          </el-button>
        </div>
      </div>
    </div>
    <div style="margin-top: 10px">
      <el-table :data="subs"
        ...
        <el-table-column prop="file.diaryTitle" label="状态" align="left">
          <template slot-scope="scope">
            <el-tag type="danger" v-if="scope.row.file.diaryTitle==null||scope.row.file.diaryTitle==''">待上传
            </el-tag>
            <el-tag type="success" v-else>已上传</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="file.diaryTitle" label="文件"></el-table-column>
        <el-table-column prop="file.diaryCreateTime" label="创建时间" align="left">
          <template slot-scope="scope">
            <p v-if="scope.row.file.diaryTitle!=null&&scope.row.file.diaryTitle!=''">
              {{ scope.row.file.diaryCreateTime }}</p>
          </template>
        </el-table-column>
        <el-table-column prop="file.diaryScore" label="成绩" align="left"></el-table-column>
        <el-table-column label="操作" fixed="right" align="left" width="200px">
          <template slot-scope="scope">
            ...
            <el-button style="padding:3px" size="mini" type="primary" @click="exportData(scope.row)"
                       v-if="scope.row.file.diaryTitle!=0&&scope.row.file.diaryTitle!=null">下载
            </el-button>
            <el-button style="padding:3px" size="mini" type="danger"
                       @click="deleteDiaryFile(scope.row)"
                       v-if="scope.row.file.diaryTitle!=0&&scope.row.file.diaryTitle!=null">删除
            </el-button>
            <el-button style="padding:3px" size="mini" type="danger"
...
export default {
  ...
  methods: {
    exportData(data){
      this.downloadRequest('/file/export/diary/?fileId='+data.id);
    },
    doUpdateScore() {
      //编辑实践日记成绩信息
    ...
    deleteDiaryFile(data) {
      this.$confirm('此操作将永久删除实践日记文件，是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.deleteRequest('/file/delete/diaryFile/' + data.id).then(resp => {
          if (resp) {
            this.initDiary();
          }
        })
    ...
    updateUrl(data) {
      this.sub = data;
      this.antionUrl = '/file/upload/diaryFile?subjectId=' + data.id + '&';
    },
